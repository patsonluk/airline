...
object AirlineCache {
  ...
  val detailedCache: LoadingCache[Int, Option[Airline]] = CacheBuilder.newBuilder.maximumSize(50000).expireAfterAccess(10, TimeUnit.MINUTES).build(new DetailedLoader())
  val simpleCache: LoadingCache[Int, Option[Airline]] = CacheBuilder.newBuilder.maximumSize(50000).expireAfterAccess(10, TimeUnit.MINUTES).build(new SimpleLoader())
