    ...
    try {  
      ...
      while (resultSet.next()) {
        ...
        while (routeEntryResultSet.next()) {
          ...
          links += Link(from = fromAirport, to = toAirport, airline = airline, price = LinkClassValues(Map.empty), distance = 0, capacity = LinkClassValues(Map.empty), rawQuality = 0, duration = 0, frequency = 0)
        }
