...
object LoanInterestRateSimulation {
  ...
  def simulate(cycle: Int) : Unit = {
    ...
    if (!rates.isEmpty) {
      logger.info(s"Interest rate simulation [previous rate : ${rates.last}, new rate : $nextRate]")
    }
  ...
  val DEFAULT_ANNUAL_RATE : Double = 0.15 //15%
  val MAX_DELTA : BigDecimal = 0.01
  val RATE_STEP : BigDecimal= 0.001 // 0.1 % is a step
  val MIN_RATE : BigDecimal= 0.02 //min rate is 2%
  val MAX_RATE = DEFAULT_ANNUAL_RATE * 2 - MIN_RATE //max rate is 28% annual
