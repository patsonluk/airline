...
class FlightPreferenceSpec(_system: ActorSystem) extends TestKit(_system) with ImplicitSender
  ...
  airline2Link.setAssignedAirplanes(scala.collection.immutable.Map(Airplane(Model.fromId(0), testAirline1, 0, purchasedCycle = 0, 100, 0, 0) -> 1))
  airline1Link.setAssignedAirplanes(scala.collection.immutable.Map(Airplane(Model.fromId(0), testAirline2, 0, purchasedCycle = 0, 100, 0, 0) -> 1))
  topAirlineLink.setAssignedAirplanes(scala.collection.immutable.Map(Airplane(Model.fromId(0), topAirline, 0, purchasedCycle = 0, 100, 0, 0) -> 1))
  ...
  "An AppealPreference".must {
    ...
    "generate similar cost if everything is the same but one has lounge but all passengers are econ".in {
      ...
      airline2Link.setAssignedAirplanes(scala.collection.immutable.Map(Airplane(Model.fromId(0), adjustedAirline1, 0, purchasedCycle = 0, 100, 0, 0) -> 1))
      airline1Link.setAssignedAirplanes(scala.collection.immutable.Map(Airplane(Model.fromId(0), adjustedAirline2, 0, purchasedCycle = 0, 100, 0, 0) -> 1))
