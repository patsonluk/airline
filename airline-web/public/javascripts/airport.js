...
function populateAirportDetails(airport) {
	...
	if (airport) {
		...
		const airportMarkerIconImage = document.createElement("img");
		airportMarkerIconImage.src = airportMarkerIcon;
...
function addMarkers(airports) {
	...
	for (i = 0; i < airports.length; i++) {
		...
		marker.map = isShowMarker(marker, currentZoom) ? map : null;
...
function addContestedMarker(airportMarker) {
	var contestedMarker = new google.maps.marker.AdvancedMarkerElement({
		...
		map,
		title: "Contested",
		content: createMapsMarkerImage("assets/images/icons/fire.png"),
		zIndex: 500
	});
...
function updateAirportBaseMarkers(newBaseAirports, relatedFlightPaths) {
	...
	$.each(baseMarkers, function (index, marker) {
		const image = createMapsMarkerImage(marker.originalIcon);
	...
	$.each(newBaseAirports, function (key, baseAirport) {
		...
		if (baseAirport.headquarter) {
			marker.content = createMapsMarkerImage(headquarterMarkerIcon);
		} else {
			marker.content = createMapsMarkerImage(baseMarkerIcon);
		}
		marker.zIndex = 999;
		marker.isBase = true;
