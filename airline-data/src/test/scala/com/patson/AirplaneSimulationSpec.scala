...
class AirplaneSimulationSpec extends WordSpecLike with Matchers {
  ...
  "decayAirplanesByAirline".must {
    "decay airplane according to airline maintenance quality".in {
       val result1 = AirplaneSimulation.decayAirplanesByAirline(Map(airplane1 -> LinkAssignments(Map(link1.id -> 1))), airline1)
       val result2 = AirplaneSimulation.decayAirplanesByAirline(Map(airplane2 -> LinkAssignments(Map(link2.id -> 1))), airline2)
    ...
    "not decay beyond 0% in lifespan".in {
       ...
       for (i <- 0 until airplane.model.lifespan) {
         airplane = AirplaneSimulation.decayAirplanesByAirline(Map(airplane -> LinkAssignments(Map(link.id -> 1))), badAirline)(0)
       }
