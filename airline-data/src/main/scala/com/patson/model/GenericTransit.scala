...
case class GenericTransit(from : Airport, to : Airport, distance : Int, var capacity: LinkClassValues, var id : Int = 0) extends Transport {
  ...
  override val cost = LinkClassValues.getInstance(economy = Pricing.computeStandardPrice(distance, FlightType.SHORT_HAUL_DOMESTIC, ECONOMY)) * 0.8 //hidden cost of general transit
  ...
  override def toString() = {
    s"General transit $id; ${from.city}(${from.iata}) => ${to.city}(${to.iata}); distance $distance"
  }
