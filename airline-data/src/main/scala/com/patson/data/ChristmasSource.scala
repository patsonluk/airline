...
object ChristmasSource {
  def saveSantaClausInfo(entries: List[SantaClausInfo]): List[SantaClausInfo] = {
    ...
    try {
      ...
      val preparedStatement = connection.prepareStatement("INSERT INTO " + SANTA_CLAUS_INFO_TABLE + "(airport, airline, attempts_left, found, picked_award) VALUES(?,?,?,?,?)")
  ...
  private def loadSantaClausInfoByQueryString(queryString : String, parameters : List[Any]) : List[SantaClausInfo] = {
    ...
    try {
        ...
        while (resultSet.next()) {
          ...
          val airport = AirportSource.loadAirportById(resultSet.getInt("airline")).getOrElse(Airport.fromId(0))
          ...
          entries += SantaClausInfo(airport, airline, attemptsLeft, found, pickedAward, id)
        }
