...
//	      }
  ...
  def computeDemandBetweenAirports(fromAirport : Airport, toAirport : Airport, relationship : Int, passengerType : PassengerType.Value) : LinkClassValues = {
    ...
    if (fromAirport == toAirport || fromAirport.population == 0 || toAirport.population == 0 || distance <= MIN_DISTANCE) {
      ...
      var baseDemand: Double = (fromAirportAdjustedPower.doubleValue() / 1000000 / 100000) * (toAirport.population.doubleValue() / 1000000 * toAirportIncomeLevel / 20) * (passengerType match {
      ...
      adjustedDemand += baseDemand * (flightType match {
        ...
        case SHORT_HAUL_INTERNATIONAL | SHORT_HAUL_INTERCONTINENTAL | MEDIUM_HAUL_INTERCONTINENTAL => 0
      ...
      val firstClassPercentage : Double = 
        if (flightType == LONG_HAUL_INTERNATIONAL || flightType == MEDIUM_HAUL_INTERCONTINENTAL || flightType == LONG_HAUL_INTERCONTINENTAL || flightType == ULTRA_LONG_HAUL_INTERCONTINENTAL || flightType == LONG_HAUL_DOMESTIC || flightType == SHORT_HAUL_INTERNATIONAL) {
  ...
  def getFlightPreferencePoolOnAirport(homeAirport : Airport) : FlightPreferencePool = {
    ...
    for (i <- 0 until 2) { //bit more randomness - set variation per group
      ...
      flightPreferences.append((AppealPreference.getAppealPreferenceWithId(homeAirport, BUSINESS, loungeLevelRequired = 3, loyaltyRatio = 2), 1))
    }
    ...
    flightPreferences.append((AppealPreference.getAppealPreferenceWithId(homeAirport, FIRST, loungeLevelRequired = 3, loyaltyRatio = 2), 1))
